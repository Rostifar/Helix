...
public:
	...
	UniverseSimulation( int _nParticles = defParticles, int _epsilon = defEpsilon, int _epochs = defEpochs, F _dt = defDt, Algorithm alg = Algorithm::NAIVE ) {
		n			= _nParticles;
		epsilon		= _epsilon;
		dt			= _dt;
		epochs		= _epochs;
...
private:
	...
	void checkSimulationParams() {
		if ( !limits.limitsSet ) {
			printf( "%s %i %s \n",
					"WARNING. No limits were defined in simulation: ", simId,
					". Configuring simulation using min values." );
			pregenerateLimits();
		}
